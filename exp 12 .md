...
```SQL
MariaDB [bhavya]> SELECT COUNT(*) AS TOTAL
    -> FROM EMPLOYEE E
    -> JOIN EMPLOYEE M ON E.MGR = M.EMPNO
    -> WHERE E.SAL > M.SAL;
+-------+
...
```SQL
MariaDB [bhavya]> DELETE FROM DEPARTMENT
    -> WHERE DEPTNO NOT IN (
...
```SQL
MariaDB [bhavya]> DELETE FROM EMPLOYEE
    -> WHERE DEPTNO NOT IN (
