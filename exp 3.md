...
```SQL
MariaDB [bhavya]> SELECT ENAME,JOB,SAL FROM EMPLOYEE
...
```sql
MariaDB [bhavya]> select ename,deptno from employee
...
```sql
MariaDB [bhavya]> select empno,(sal+ifnull(comm,0)) as total_salary
    -> from employee;
+-------+--------------+
| empno | total_salary |
+-------+--------------+
...
```sql
MariaDB [bhavya]> select empno,(sal*12) as annual_salary
    -> from employee;
+-------+---------------+
| empno | annual_salary |
+-------+---------------+
