...
```SQL
MariaDB [bhavya]> select empno,ename,
    -> case deptno
...
```sql
MariaDB [bhavya]> select timestampdiff(day,'2006-02-10',curdate());
+-------------------------------------------+
...
## 3. age in months.
```sql
MariaDB [bhavya]> select timestampdiff(month,'2006-02-10',curdate());
+---------------------------------------------+
...
```sql
MariaDB [bhavya]> select date_format(curdate(),'%d %m %w %y') as curent_date;
+-------------+
| curent_date |
+-------------+
| 17 02 2 26  |
...
```sql
MariaDB [bhavya]> select concat(ename ,'  has joined the company on  ',
...
```sql
MariaDB [bhavya]> select date_add(curdate(),interval 3 day);
+------------------------------------+
...
```sql
MariaDB [bhavya]> select date_sub(curdate(),interval 3 month);
+--------------------------------------+
...
```sql
MariaDB [bhavya]> select ename,hiredate
    -> from employee
...
```sql
MariaDB [bhavya]> select ename,(sal*0.10)as salary,year(hiredate)
    -> from employee
...
```sql
MariaDB [bhavya]> select ename, hiredate from employee
...
```sql
MariaDB [bhavya]> select ename,hiredate,deptno
    -> from employee
