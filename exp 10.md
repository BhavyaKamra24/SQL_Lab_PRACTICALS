...
```sql
MariaDB [bhavya]> SELECT e.Ename,d.Dname,e.SAL,e.COMM
    -> FROM Employee e
    -> JOIN Department d
    -> ON e.Deptno = d.Deptno
...
## Q8.Display those employees whose salary greater thaqn his manager salary.
```sql
MariaDB [bhavya]> SELECT e.Ename
    -> FROM Employee e
    -> JOIN Employee M
...
```sql
MariaDB [bhavya]> SELECT e.Ename,S.GRADE
    -> FROM Employee e
    -> JOIN SALGRADE S
